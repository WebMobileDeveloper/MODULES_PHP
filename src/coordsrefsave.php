...
if (!$rowr) {  // si reference existe pas alors on va faire l'insertion dans coords
  ...
  foreach ($tablename as $precision) {
    ...
    $resul = $mysql->query("SELECT id,ref FROM Coords$precision WHERE ref = $ref");
    $rowr = $resul->fetch(PDO::FETCH_ASSOC);

    if (!$rowr) {

      $statement = $mysql->prepare("INSERT INTO Coords$precision (lat,lon,name,numero,villeid,ref)
			    VALUES(:lat, :lon, :name, :numero, :villeid, :ref)");
  ...
  // for each precision, update the information
  foreach ($tablename as $precision) {
    $main_range = $precision; // the max allowed random range
    $mini_range = $precision / 2; // the min allowed range
    $sql = "SELECT * FROM Coords$precision WHERE ref = $ref";
    $resx = $mysql->query($sql);
    while ($row = $resx->fetch(PDO::FETCH_ASSOC)) {
      $ref = $row['ref'];
      $lat = $row['lat'];
      $lon = $row['lon'];
      do {
      ...
      $lat = $res['lat'];
      $lon = $res['lon'];
      $update_sql2 = "UPDATE Coords$precision SET lat=:lat, lon=:lon WHERE ref=:ref";
      $stmt = $mysql->prepare($update_sql2);
      $stmt->execute(array(':lat' => $res['lat'], ':lon' => $res['lon'], ':ref' => $ref));
    }
  }
}
